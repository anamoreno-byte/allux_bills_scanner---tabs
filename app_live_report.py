...
def aplicar_override_iberdrola_liverpool(parsed: pd.DataFrame) -> pd.DataFrame:
    """
    Ajuste manual para recibos de Iberdrola.

    Si source_utility = IBERDROLA, asignamos los nombres de DG/DGH
    para que el match global pueda cruzar contra el local correcto.
    ...
    # Asegurar columnas que usa el match global
    for col in [
        "mall_folder",
        "cliente_nombre",
        "recibos_subgroup",
    ...
    # Centro comercial
    # Usamos exactamente el mismo mall_folder que ya existe en el parser
    # para Ambar, para evitar que aparezca como un segundo centro comercial.

    ambar_mall_folder = "Ambar Fashion Mall Tuxtla"
    ...
    if "mall_folder" in p.columns:
        ...
        if not candidatos_ambar.empty:
            ambar_mall_folder = candidatos_ambar.mode().iloc[0]
    ...
    # Nombre comercial DG/DGH
    p.loc[
        mask_iberdrola,
        "recibos_subgroup"
    ] = "LIVERPOOL"

    # Respaldos visuales por si otras secciones usan estas columnas
    p.loc[
        mask_iberdrola,
        "tenant"
    ] = "LIVERPOOL"
    ...
    # Columnas auxiliares para poder auditar que fue un override manual
    p.loc[
        mask_iberdrola,
        "override_match_dg"
    ] = "IBERDROLA → LIVERPOOL / OPERADORA DE ALMACENES LIVERPOOL"
...
def construir_muestra_con_recibo_global(parsed, general_data, mall_col):
    ...
    for mall_name in sorted(parsed[mall_col].dropna().unique()):
        ...
        # coincidió por CLIENTE o NOMBRE COMERCIAL de forma amplia.
        #
        # Debe contar como con recibo únicamente si el match logró copiar
        # información concreta del parser:
        # - no_servicio
        # - medidor
        # - tarifa
        # - file_path
        ...
        # --------------------------------------------------
        # Primero marcamos como no confirmados los locales que parecían
        # tener recibo por cliente/nombre/medidor/local, pero a los que
        # NO se les copió información concreta del parser.
        #
        # Después refinamos:
        # Si el único candidato disponible en parser ya fue asignado a
        ...
        # - Bershka ya tomó no_servicio X
        # - Stradivarius ya tomó no_servicio Y
        ...
        def servicios_candidatos_parser_para_local(row):
            ...
            Esta función NO confirma match.
            Solo sirve para saber si todavía hay algún no_servicio disponible
        ...
        # si todos sus servicios candidatos ya están asignados a otros locales,
        ...
        # A partir de aquí, la muestra global usa solo matches confirmados.
        general_ocupados_cc["_tiene_recibo"] = (
            general_ocupados_cc["_tiene_recibo_confirmado"]
        )
